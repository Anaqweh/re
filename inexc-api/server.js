...
app.get('/:shareSlug', async (req, res, next) => {
  try {
    ...
    const panels = [
      ...
      ...(course.axes.length ? [{ label: 'محاور الدورة', html: `<section class="panel"><h2>محاور الدورة</h2><p class="lead">تعرف على محتوى الدورة ومحاورها العملية خطوة بخطوة.</p>${course.axes.map((axis, index) => `<article class="axis"><b><i>${['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠'][index] || index + 1}</i><span>${publicInlineText(axis)}</span></b></article>`).join('')}</section>` }] : []),
