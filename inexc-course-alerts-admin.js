...
(() => {
  ...
  async function loadAlertCourses() {
    const select = section.querySelector('#alertNowCourse');
    try {
      const courses = await call('/admin/courses');
      const selected = select.value;
      const published = courses.filter(course => course.active);
      select.innerHTML = '<option value="">اختر الدورة المنشورة</option>' + published.map(course => '<option value="' + escape(course.id) + '">' + escape(course.name) + '</option>').join('');
      if (published.some(course => course.id === selected)) select.value = selected;
    } catch (error) { select.innerHTML = '<option value="">تعذر تحميل الدورات</option>'; }
  }
  ...
  tab.addEventListener('click', () => {
    ...
    tab.classList.add('active'); section.classList.add('active'); loadAlerts(); loadAlertSettings(); loadAlertCourses();
  });
  ...
  section.querySelector('#alertSendNowForm').addEventListener('submit', async event => {
    event.preventDefault();
    const courseId = section.querySelector('#alertNowCourse').value;
    const count = Number(section.querySelector('#alertNowCount').value);
    const button = event.target.querySelector('button');
    const output = section.querySelector('#alertSendNowResult');
    if (!courseId) { output.style.color='var(--bad)'; output.textContent='اختر الدورة أولًا.'; return; }
    if (!confirm('سيتم إرسال قالب التنبيه الحالي إلى الدفعة المختارة للدورة المحددة. هل تريد المتابعة؟')) return;
    button.disabled = true; button.textContent = 'جارٍ تجهيز الإرسال…'; output.textContent = '';
    try {
      const result = await call('/admin/course-alerts/send-now', { method:'POST', body:JSON.stringify({ courseId, count }) });
      output.style.color='#168865';
      output.textContent = result.queued ? 'تمت جدولة ' + result.queued + ' رسالة الآن لدورة «' + result.courseName + '».': 'لا توجد عناوين جديدة لم يصلها تنبيه هذه الدورة.';
      loadAlerts();
    } catch (error) { output.style.color='var(--bad)'; output.textContent=error.message; }
    finally { button.disabled=false; button.textContent='إرسال القالب الآن'; }
  });
  ...
  const singleSendForm = section.querySelector('#alertSingleSendForm');
  section.addEventListener('click', async event => {
    const send = event.target.closest('[data-send-alert]');
    const stop = event.target.closest('[data-delete-alert]');
    if (send) {
      const select = section.querySelector('#singleAlertCourse');
      const output = section.querySelector('#singleAlertSendResult');
      singleSendForm.hidden = false;
      singleSendForm.dataset.subscriberId = send.dataset.sendAlert;
      section.querySelector('#singleSendEmail').textContent = 'سيتم الإرسال إلى: ' + send.dataset.alertEmail;
      select.innerHTML = '<option value="">جارٍ تحميل الدورات…</option>';
      output.textContent = '';
      try {
        const courses = await call('/admin/courses');
        const published = courses.filter(course => course.active);
        select.innerHTML = '<option value="">اختر الدورة</option>' + published.map(course => '<option value="' + escape(course.id) + '">' + escape(course.name) + '</option>').join('');
        if (!published.length) output.textContent = 'لا توجد دورة منشورة حاليًا.';
      } catch (error) { select.innerHTML = '<option value="">تعذر تحميل الدورات</option>'; output.textContent = error.message; }
      singleSendForm.scrollIntoView({ behavior:'smooth', block:'nearest' });
      return;
    }
    if (!stop) return;
    ...
    stop.disabled = true;
    try { await call('/admin/course-alerts/' + stop.dataset.deleteAlert, { method:'DELETE' }); loadAlerts(); }
    catch (error) { alert(error.message); stop.disabled = false; }
  });
  section.querySelector('#cancelSingleAlertSend').addEventListener('click', () => { singleSendForm.hidden = true; });
  singleSendForm.addEventListener('submit', async event => {
    event.preventDefault();
    const courseId = section.querySelector('#singleAlertCourse').value;
    const output = section.querySelector('#singleAlertSendResult');
    const button = event.target.querySelector('button[type="submit"]');
    if (!courseId) { output.style.color='var(--bad)'; output.textContent='اختر الدورة أولًا.'; return; }
    if (!confirm('سيتم إرسال قالب التنبيه الحالي إلى هذا البريد للدورة المختارة فقط. هل تريد المتابعة؟')) return;
    button.disabled = true; button.textContent = 'جارٍ الإرسال…'; output.textContent = '';
    try {
      const result = await call('/admin/course-alerts/' + singleSendForm.dataset.subscriberId + '/send', { method:'POST', body:JSON.stringify({ courseId }) });
      output.style.color='#168865'; output.textContent='تم إرسال تنبيه دورة «' + result.courseName + '» بنجاح.';
      loadAlerts();
    } catch (error) { output.style.color='var(--bad)'; output.textContent=error.message; }
    finally { button.disabled=false; button.textContent='إرسال بالقالب الحالي'; }
  });
})();
