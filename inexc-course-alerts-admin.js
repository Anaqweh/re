...
(() => {
  ...
  section.querySelector('#alertSettingsForm').addEventListener('submit', async event => {
    ...
    try {
      ...
      output.style.color='#168865'; output.textContent = saved.enabled ? 'تم تفعيل الإرسال التلقائي وحفظ وقت الإرسال.' : 'تم تعطيل الإرسال التلقائي مؤقتًا.';
