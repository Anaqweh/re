    ...
    try {
      if (institutional) {
        const response = await fetch(`${endpoint}/institution-requests`, {
          ...
          body: JSON.stringify({ name: document.getElementById('name').value.trim(), email: document.getElementById('email').value.trim(), phone: document.getElementById('phone').value.trim(), organization: document.getElementById('organization').value.trim(), audienceSize: document.getElementById('audienceSize').value.trim(), certificateInterest: document.getElementById('certificateInterest').value, requestDetails: document.getElementById('institutionRequest').value.trim(), preferredTiming: document.getElementById('preferredTiming').value.trim() })
        });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'تعذر إرسال طلب المؤسسة.');
        form.style.display = 'none'; document.getElementById('success').classList.add('show'); document.getElementById('reference').textContent = `رقم طلبكم: ${result.reference}`; document.getElementById('successText').textContent = 'تم استلام طلب مؤسستكم بنجاح. سيتواصل فريق INEXC معكم قريبًا لمناقشة البرنامج المناسب.'; return;
      }
