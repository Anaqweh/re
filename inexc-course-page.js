(() => {
  ...
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      ...
      const panels = [
        { label:'عن الدورة', content:`<section class="panel intro"><span class="tag">${esc(course.category || 'دورة تدريبية')}</span><h1>${esc(course.name)}</h1><div class="rich">${rich(course.description)}</div><div class="meta"><span>📅 ${esc(course.date || 'سيُعلن قريبًا')}</span><span>📍 ${esc(course.location || 'عن بُعد / حضوري')}</span>${Number(course.hours) > 0 ? `<span>⏱️ ${Number(course.hours).toLocaleString('ar-AE')} ساعة تدريبية</span>` : ''}<span>💳 ${Number(course.price) === 0 ? 'مجاني' : `${Number(course.price).toLocaleString('ar-AE')} د.إ`}</span></div><button class="share-button" type="button">مشاركة الدورة</button></section>` },
        ...
        { label:'هل تناسبك؟', content:`<section class="panel"><h2>هل هذه الدورة مناسبة لك؟</h2><p class="panel-lead">أجب عن 3 أسئلة قصيرة لتحصل على توجيه مناسب قبل التسجيل.</p><div class="fit-card" data-fit-check><div class="fit-question"><b>1. ما هدفك الأساسي من الدورة؟</b><div class="fit-options"><button class="fit-option" type="button" data-fit-question="goal" data-fit-value="تطوير مهارة عملية">تطوير مهارة عملية</button><button class="fit-option" type="button" data-fit-question="goal" data-fit-value="تطبيقها في التعليم أو العمل">تطبيقها في التعليم أو العمل</button><button class="fit-option" type="button" data-fit-question="goal" data-fit-value="بناء مشروع أو مخرج واضح">بناء مشروع أو مخرج واضح</button></div></div><div class="fit-question"><b>2. ما مستوى خبرتك الحالي؟</b><div class="fit-options"><button class="fit-option" type="button" data-fit-question="level" data-fit-value="beginner">مبتدئ</button><button class="fit-option" type="button" data-fit-question="level" data-fit-value="basic">لدي معرفة أساسية</button><button class="fit-option" type="button" data-fit-question="level" data-fit-value="advanced">أستخدم الأدوات وأريد مستوى متقدمًا</button></div></div><div class="fit-question"><b>3. كيف ستستفيد منها؟</b><div class="fit-options"><button class="fit-option" type="button" data-fit-question="use" data-fit-value="عملك الحالي">في عملي الحالي</button><button class="fit-option" type="button" data-fit-question="use" data-fit-value="تدريبك أو تدريسك">في تدريبي أو تدريسي</button><button class="fit-option" type="button" data-fit-question="use" data-fit-value="مشروع شخصي أو مؤسسي">في مشروع شخصي أو مؤسسي</button></div></div><div class="fit-result" data-fit-result>أجب عن الأسئلة الثلاثة لتظهر لك النتيجة المناسبة.</div><a class="fit-register" data-fit-register href="/register/?course=${encodeURIComponent(course.name)}">سجّل في الدورة الآن ←</a></div></section>` },
      ...
      if (fitCheck) {
        const answers = {}, result = fitCheck.querySelector('[data-fit-result]'), register = fitCheck.querySelector('[data-fit-register]');
        fitCheck.querySelectorAll('[data-fit-question]').forEach(button => button.addEventListener('click', () => {
          const question = button.dataset.fitQuestion;
          answers[question] = button.dataset.fitValue;
          fitCheck.querySelectorAll(`[data-fit-question="${question}"]`).forEach(item => item.classList.toggle('selected', item === button));
          if (answers.goal && answers.level && answers.use) {
            const levelNote = answers.level === 'beginner' ? 'وستحصل على انتقال تدريجي من الفهم إلى التطبيق.' : answers.level === 'advanced' ? 'وستساعدك على الانتقال إلى مستوى أكثر تقدمًا في التطبيق والإنتاج.' : 'وستبني على معرفتك الحالية بخطوات عملية واضحة.';
            result.innerHTML = `<strong>هذه الدورة مناسبة لك ✓</strong><br>لأن هدفك هو ${esc(answers.goal)}، وستوظف ما تتعلمه في ${esc(answers.use)}. ${levelNote}`;
            register.classList.add('ready');
          }
        }));
      }
